...
- CLI/version: bake the package version into the bundle at build time via `tsdown` define so `openclaw --version` reliably reports the correct version in Homebrew and other installs where runtime `package.json` lookup can fail. (#35768)
